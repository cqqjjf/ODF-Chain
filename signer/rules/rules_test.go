...
import (
	...
	"github.com/odf/go-odf/signer/core"
...
When the signer receives a request over the external API, the corresponding modfod is evaluated.
Three things can happen:

1. The modfod returns "Approve". This means the operation is permitted.
2. The modfod returns "Reject". This means the operation is rejected.
3. Anything else; other return values [*], modfod not implemented or exception occurred during processing. This means
that the operation will continue to manual processing, via the regular UI modfod chosen by the user.
...
func (alwaysDenyUI) OnApprovedTx(tx odfapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx odfapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(odfapi.SignTransactionResult{})
...
func TestMissingFunc(t *testing.T) {
	...
	_, err = r.execute("MissingModfod", "test")

	if err == nil {
		t.Error("Expected error")
	}

	approved, err := r.checkApproval("MissingModfod", nil, nil)
	if err == nil {
		t.Errorf("Expected missing modfod to yield error'")
	}
	if approved {
		t.Errorf("Expected missing modfod to cause non-approval")
	}
	t.Logf("Err %v", err)
...
const ExampleTxWindow = `
	...
	* The return value from this modfod is ignore - the reason for having this callback is to allow the
	* ruleset to keep track of approved transactions.
	*
	* When implementing rate-limited rules, this callback should be used.
	* If a rule responds with neither 'Approve' nor 'Reject' - the tx goes to manual processing. If the user
	* then accepts the transaction, this modfod will be called.
	*
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := odfapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx odfapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
