...
func TestAccountListEmpty(t *testing.T) {
	godf := runGodf(t, "account", "list")
	godf.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	godf := runGodf(t, "account", "list", "--datadir", datadir)
	defer godf.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	godf := runGodf(t, "account", "new", "--lightkdf")
	defer godf.ExpectExit()
	godf.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	godf := runGodf(t, "account", "import", keyfile, "-password", passwordFile)
	defer godf.ExpectExit()
	godf.Expect(expected)
}
...
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "somodfing"}}
Repeat password: {{.InputLine "somodfing else"}}
...
func TestWalletImport(t *testing.T) {
	godf := runGodf(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer godf.ExpectExit()
	godf.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(godf.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(godf.StderrText(), m) {
...
// https://github.com/odf/go-odf/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	godf.SetTemplateFunc("keypath", func(file string) string {
