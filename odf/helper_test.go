...
func newTestProtocolManager(mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, odfdb.Database, error) {
	var (
	...
	chain, _ := core.GenerateChain(gspec.Config, genesis, odfash.NewFaker(), db, blocks, generator)
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, odfdb.Database) {
...
// Has returns an indicator whodfer txpool has a transaction
...
func (p *testPeer) handshake(t *testing.T, td *big.Int, head common.Hash, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter) {
	...
	default:
		panic(fmt.Sprintf("unsupported odf protocol version: %d", p.version))
	}
