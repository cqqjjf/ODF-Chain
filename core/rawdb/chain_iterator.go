...
// of frozen ancient blocks. The modfod iterates over all the frozen blocks and
// injects into the database the block hash->number mappings.
func InitDatabaseFromFreezer(db odfdb.Database) {
...
func iterateTransactions(db odfdb.Database, from uint64, to uint64, reverse bool, interrupt chan struct{}) chan *blockTxHashes {
...
// signal received.
func indexTransactions(db odfdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func IndexTransactions(db odfdb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func indexTransactionsForTesting(db odfdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func unindexTransactions(db odfdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
	// short circuit for invalid range
	if from >= to {
		return
	}
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func UnindexTransactions(db odfdb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func unindexTransactionsForTesting(db odfdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
