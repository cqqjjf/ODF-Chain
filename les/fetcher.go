...
// the odf.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb odfdb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
...
func (f *lightFetcher) mainloop() {
	...
	var (
		...
		syncing      bool        // Indicator whodfer the client is syncing
	...
	// trustedHeader returns an indicator whodfer the header is regarded as
...
//
// Note, we rely on the underlying odf/fetcher to retrieve and validate the
// response, so that we have to obey the rule of odf/fetcher which only accepts
