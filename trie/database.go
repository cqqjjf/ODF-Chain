...
type Database struct {
	diskdb odfdb.KeyValueStore // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb odfdb.KeyValueStore) *Database {
...
func NewDatabaseWithCache(diskdb odfdb.KeyValueStore, cache int, journal string) *Database {
...
func (db *Database) DiskDB() odfdb.KeyValueStore {
	return db.diskdb
}
...
// yet unknown. The modfod will NOT make a copy of the slice,
// only use if the preimage will NOT be changed later on.
//
// Note, this modfod assumes that the database's lock is held!
...
// cached, the modfod queries the persistent database for the content.
...
// found cached, the modfod queries the persistent database for the content.
...
// This modfod is extremely expensive and should only be used to validate internal
// states in test code.
...
// are referenced togodfer by database itself.
...
//
// Note, this modfod is a non-synchronized mutator. It is unsafe to call this
...
func (db *Database) commit(hash common.Hash, batch odfdb.Batch, uncacher *cleaner, callback func(common.Hash)) error {
