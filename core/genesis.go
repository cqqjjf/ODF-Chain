...
func SetupGenesisBlock(db odfdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	...
	// Check whodfer the genesis block is already written.
	if genesis != nil {
		hash := genesis.ToBlock(nil).Hash()
		if hash != stored {
			return genesis.Config, hash, &GenesisMismatchError{stored, hash}
...
func (g *Genesis) ToBlock(db odfdb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db odfdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db odfdb.Database) *types.Block {
...
func GenesisBlockForTesting(db odfdb.Database, addr common.Address, balance *big.Int) *types.Block {
...
// DeveloperGenesisBlock returns the 'godf --dev' genesis block.
