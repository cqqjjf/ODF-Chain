...
func (w *wizard) makeGenesis() {
	...
	switch {
	case choice == "1":
		// In case of odfash, we're pretty much done
...
// importGenesis imports a Godf genesis spec into puppodf.
...
func (w *wizard) manageGenesis() {
	// Figure out whodfer to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alodf.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppodf and Godf
		godfJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((godfJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", godfJson)

		// Export the genesis spec used by Alodf (formerly C++ Ethereum)
		if spec, err := newAlodfGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Alodf chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alodf", spec)
		}
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not somodfing I can do")
		return
	}
}
