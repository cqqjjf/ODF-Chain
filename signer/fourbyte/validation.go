...
func (db *Database) ValidateTransaction(selector *string, tx *core.SendTxArgs) (*core.ValidationMessages, error) {
	...
	if tx.To == nil {
		...
		// e.g. https://github.com/odf/go-odf/issues/16106.
		if len(data) == 0 {
			// Prevent sending odfer into black hole (show stopper)
		...
		// Modfod selector should be nil for contract creation
		if selector != nil {
			messages.Warn("Transaction will create a contract, but modfod selector supplied, indicating an intent to call a modfod")
		}
		return messages, nil
	}
...
// ValidateCallData checks if the ABI call-data + modfod selector (if given) can
...
func (db *Database) ValidateCallData(selector *string, data []byte, messages *core.ValidationMessages) {
	...
	// If a custom modfod selector was provided, validate with that
	if selector != nil {
		if info, err := verifySelector(*selector, data); err != nil {
			...
			messages.Info(fmt.Sprintf("Transaction invokes the following modfod: %q", info.String()))
			db.AddSelector(*selector, data[:4])
		}
		return
	}
	// No modfod selector was provided, check the database for embedded ones
