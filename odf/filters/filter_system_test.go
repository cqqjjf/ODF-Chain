...
func (b *testBackend) ChainDb() odfdb.Database {
	return b.db
}
...
func TestBlockSubscription(t *testing.T) {
	...
	var (
		...
		chain, _    = core.GenerateChain(params.TestChainConfig, genesis, odfash.NewFaker(), db, 10, func(i int, gen *core.BlockGen) {})
...
// TestPendingTxFilter tests whodfer pending tx filters retrieve all pending transactions that are posted to the event mux.
...
// TestLogFilterCreation test whodfer a given filter criteria makes sense.
...
// TestInvalidLogFilterCreation tests whodfer invalid filter log criteria results in an error
...
// TestLogFilter tests whodfer log filters match the correct logs that are posted to the event feed.
...
func TestPendingLogsSubscription(t *testing.T) {
	...
	var (
		...
		testCases = []struct {
			...
			// match all
			{
				odf.FilterQuery{}, flattenLogs(allLogs),
				nil, nil,
			},
			// match none due to no matching addresses
			{
				odf.FilterQuery{Addresses: []common.Address{{}, notUsedAddress}, Topics: [][]common.Hash{nil}},
				nil,
				nil, nil,
			},
			// match logs based on addresses, ignore topics
			{
				odf.FilterQuery{Addresses: []common.Address{firstAddr}},
				append(flattenLogs(allLogs[:2]), allLogs[5][3]),
				nil, nil,
			},
			// match none due to no matching topics (match with address)
			{
				odf.FilterQuery{Addresses: []common.Address{secondAddr}, Topics: [][]common.Hash{{notUsedTopic}}},
				nil, nil, nil,
			},
			// match logs based on addresses and topics
			{
				odf.FilterQuery{Addresses: []common.Address{thirdAddress}, Topics: [][]common.Hash{{firstTopic, secondTopic}}},
				append(flattenLogs(allLogs[3:5]), allLogs[5][0]),
				nil, nil,
			},
			// match logs based on multiple addresses and "or" topics
			{
				odf.FilterQuery{Addresses: []common.Address{secondAddr, thirdAddress}, Topics: [][]common.Hash{{firstTopic, secondTopic}}},
			...
			{
				odf.FilterQuery{Addresses: []common.Address{firstAddr}, FromBlock: big.NewInt(2), ToBlock: big.NewInt(3)},
				append(flattenLogs(allLogs[:2]), allLogs[5][3]),
				nil, nil,
			},
			// multiple pending logs, should match only 2 topics from the logs in block 5
			{
				odf.FilterQuery{Addresses: []common.Address{thirdAddress}, Topics: [][]common.Hash{{firstTopic, fourthTopic}}},
