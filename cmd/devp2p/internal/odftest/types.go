...
import (
	...
	"github.com/odf/go-odf/internal/utesting"
	"github.com/odf/go-odf/p2p"
	"github.com/odf/go-odf/p2p/rlpx"
...
// Status is the network packet for the status message for odf/64 and later.
type Status struct {
...
func (c *Conn) handshake(t *utesting.T) Message {
	...
	case *Hello:
		...
		if c.odfProtocolVersion == 0 {
			t.Fatalf("unexpected odf protocol version")
		}
		return msg
	default:
...
// negotiateEthProtocol sets the Conn's odf protocol version
// to highest advertised capability from peer
func (c *Conn) negotiateEthProtocol(caps []p2p.Cap) {
	var highestEthVersion uint
	for _, capability := range caps {
		if capability.Name != "odf" {
			continue
		}
	...
	c.odfProtocolVersion = highestEthVersion
}
...
loop:
	...
	// make sure odf protocol version is set for negotiation
	if c.odfProtocolVersion == 0 {
		t.Fatalf("odf protocol version must be set in Conn")
	}
	// write status message to client
	status := Status{
		ProtocolVersion: uint32(c.odfProtocolVersion),
		NetworkID:       1,
