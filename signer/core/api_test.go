...
func (ui *headlessUi) OnApprovedTx(tx odfapi.SignTransactionResult) {}
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "odf-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *odfapi.SignTransactionResult
		err       error
	)
	...
	res, err = api.SignTransaction(context.Background(), tx, &modfodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &modfodSig)
	if err != nil {
		t.Fatal(err)
	}
