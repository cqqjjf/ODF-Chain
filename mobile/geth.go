...
// NodeConfig represents the collection of configuration values to fine tune the Godf
// node embedded into a mobile process. The available values are a subset of the
// entire API provided by go-odf to reduce the maintenance surface and dev
// complexity.
type NodeConfig struct {
	...
	// EthereumEnabled specifies whodfer the node should run the Ethereum protocol.
...
// Node represents a Godf Ethereum node instance.
type Node struct {
	node *node.Node
}

// NewNode creates and configures a new Godf node.
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.EthereumEnabled {
		odfConf := odf.DefaultConfig
		odfConf.Genesis = genesis
		odfConf.SyncMode = downloader.LightSync
		odfConf.NetworkId = uint64(config.EthereumNetworkID)
		odfConf.DatabaseCache = config.EthereumDatabaseCache
		lesBackend, err := les.New(rawStack, &odfConf)
		if err != nil {
			return nil, fmt.Errorf("odf init: %v", err)
		}
		// If netstats reporting is requested, do it
		if config.EthereumNetStats != "" {
			if err := odfstats.New(rawStack, lesBackend.ApiBackend, lesBackend.Engine(), config.EthereumNetStats); err != nil {
...
func (n *Node) GetEthereumClient() (client *EthereumClient, _ error) {
	...
	return &EthereumClient{odfclient.NewClient(rpc)}, nil
}
