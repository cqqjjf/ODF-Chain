...
//
// This modfod is legacy support for initial transaction exchange in odf/64 and
// prior. For odf/65 and higher use SendPooledTransactionHashes.
...
// in its transaction hash set for future reference.
//
// This modfod is a helper used by the async transaction sender. Don't call it
...
//
// This modfod is a helper used by the async transaction announcer. Don't call it
// directly as the queueing (memory) and transmission (bandwidth) costs should
// not be managed directly.
...
//
// Note, the modfod assumes the hashes are correct and correspond to the list of
...
// Handshake executes the odf protocol handshake, negotiating version number,
