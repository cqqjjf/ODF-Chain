...
// Package filters implements an odf filtering system for block,
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast modfod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this modfod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this modfod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit odf.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit odf.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit odf.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit odf.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
