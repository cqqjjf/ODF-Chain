...
const (
	...
	odfVersion        = 63              // equivalent odf version for the downloader
...
type serverHandler struct {
	...
	synced  func() bool    // Callback function used to determine whodfer local node is synced.

	// Testing fields
	addTxsSync bool
}

func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb odfdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light odf response", "err", err)
			return err
		default:
		}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	// accept returns an indicator whodfer the request can be served.
