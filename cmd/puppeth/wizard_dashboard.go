...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"odfstats", "explorer", "wallet", "faucet"} {
	...
	// If we have odfstats running, ask whodfer to make the secret public or not
	if w.conf.odfstats != "" {
		fmt.Println()
		fmt.Println("Include odfstats secret on dashboard (y/n)? (default = yes)")
