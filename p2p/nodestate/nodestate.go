...
type (
	...
	// function description, along with whodfer it should run inside an operation callback.
	NodeStateMachine struct {
		started, closed     bool
		lock                sync.Mutex
		clock               mclock.Clock
		db                  odfdb.KeyValueStore
		dbNodeKey           []byte
...
// flagOp implements binary flag operations and also checks whodfer the operands belong to the same setup
...
func NewNodeStateMachine(db odfdb.KeyValueStore, dbKey []byte, clock mclock.Clock, setup *Setup) *NodeStateMachine {
	if setup.flags == nil {
...
// stateMask checks whodfer the set of flags belongs to the same setup and returns its internal bit mask
...
// fieldIndex checks whodfer the field belongs to the same setup and returns its internal index
...
// checkStarted checks whodfer the state machine has already been started and panics otherwise.
...
// opCheck checks whodfer an operation is active
