...
// Package utils contains internal helper functions for go-odf commands.
package utils
...
import (
	...
	"github.com/odf/go-odf/consensus/clique"
	...
	"github.com/odf/go-odf/odfstats"
	"github.com/odf/go-odf/graphql"
	"github.com/odf/go-odf/internal/odfapi"
	"github.com/odf/go-odf/internal/flags"
	"github.com/odf/go-odf/les"
	"github.com/odf/go-odf/log"
	"github.com/odf/go-odf/metrics"
	"github.com/odf/go-odf/metrics/exp"
	"github.com/odf/go-odf/metrics/influxdb"
	"github.com/odf/go-odf/miner"
	"github.com/odf/go-odf/node"
	"github.com/odf/go-odf/p2p"
	"github.com/odf/go-odf/p2p/discv5"
	"github.com/odf/go-odf/p2p/enode"
	"github.com/odf/go-odf/p2p/nat"
	"github.com/odf/go-odf/p2p/netutil"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: odf.DefaultConfig.NetworkId,
	}
	...
	defaultSyncMode = odf.DefaultConfig.SyncMode
	...
	LightServeFlag = cli.IntFlag{
		...
		Value: odf.DefaultConfig.LightServ,
	}
	LightIngressFlag = cli.IntFlag{
		Name:  "light.ingress",
		Usage: "Incoming bandwidth limit for serving light clients (kilobytes/sec, 0 = unlimited)",
		Value: odf.DefaultConfig.LightIngress,
	}
	LightEgressFlag = cli.IntFlag{
		Name:  "light.egress",
		Usage: "Outgoing bandwidth limit for serving light clients (kilobytes/sec, 0 = unlimited)",
		Value: odf.DefaultConfig.LightEgress,
	}
	LightMaxPeersFlag = cli.IntFlag{
		Name:  "light.maxpeers",
		Usage: "Maximum number of light clients to serve, or light servers to attach to",
		Value: odf.DefaultConfig.LightPeers,
	}
	UltraLightServersFlag = cli.StringFlag{
		Name:  "ulc.servers",
		Usage: "List of trusted ultra-light servers",
		Value: strings.Join(odf.DefaultConfig.UltraLightServers, ","),
	}
	UltraLightFractionFlag = cli.IntFlag{
		Name:  "ulc.fraction",
		Usage: "Minimum % of trusted ultra-light servers required to announce a new head",
		Value: odf.DefaultConfig.UltraLightFraction,
	}
	...
	EthashCacheDirFlag = DirectoryFlag{
		Name:  "odfash.cachedir",
		Usage: "Directory to store the odfash verification caches (default = inside the datadir)",
	}
	EthashCachesInMemoryFlag = cli.IntFlag{
		Name:  "odfash.cachesinmem",
		Usage: "Number of recent odfash caches to keep in memory (16MB each)",
		Value: odf.DefaultConfig.Ethash.CachesInMem,
	}
	EthashCachesOnDiskFlag = cli.IntFlag{
		Name:  "odfash.cachesondisk",
		Usage: "Number of recent odfash caches to keep on disk (16MB each)",
		Value: odf.DefaultConfig.Ethash.CachesOnDisk,
	}
	EthashCachesLockMmapFlag = cli.BoolFlag{
		Name:  "odfash.cacheslockmmap",
		Usage: "Lock memory maps of recent odfash caches",
	}
	EthashDatasetDirFlag = DirectoryFlag{
		Name:  "odfash.dagdir",
		Usage: "Directory to store the odfash mining DAGs",
		Value: DirectoryString(odf.DefaultConfig.Ethash.DatasetDir),
	}
	EthashDatasetsInMemoryFlag = cli.IntFlag{
		Name:  "odfash.dagsinmem",
		Usage: "Number of recent odfash mining DAGs to keep in memory (1+GB each)",
		Value: odf.DefaultConfig.Ethash.DatasetsInMem,
	}
	EthashDatasetsOnDiskFlag = cli.IntFlag{
		Name:  "odfash.dagsondisk",
		Usage: "Number of recent odfash mining DAGs to keep on disk (1+GB each)",
		Value: odf.DefaultConfig.Ethash.DatasetsOnDisk,
	}
	EthashDatasetsLockMmapFlag = cli.BoolFlag{
		Name:  "odfash.dagslockmmap",
		Usage: "Lock memory maps for recent odfash mining DAGs",
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: odf.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: odf.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: odf.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: odf.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: odf.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: odf.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: odf.DefaultConfig.TxPool.Lifetime,
	}
	...
	CacheTrieJournalFlag = cli.StringFlag{
		...
		Value: odf.DefaultConfig.TrieCleanCacheJournal,
	}
	CacheTrieRejournalFlag = cli.DurationFlag{
		Name:  "cache.trie.rejournal",
		Usage: "Time interval to regenerate the trie cache journal",
		Value: odf.DefaultConfig.TrieCleanCacheRejournal,
	}
	...
	MinerGasTargetFlag = cli.Uint64Flag{
		...
		Value: odf.DefaultConfig.Miner.GasFloor,
	}
	MinerGasLimitFlag = cli.Uint64Flag{
		Name:  "miner.gaslimit",
		Usage: "Target gas ceiling for mined blocks",
		Value: odf.DefaultConfig.Miner.GasCeil,
	}
	MinerGasPriceFlag = BigFlag{
		Name:  "miner.gasprice",
		Usage: "Minimum gas price for mining a transaction",
		Value: odf.DefaultConfig.Miner.GasPrice,
	}
	MinerEtherbaseFlag = cli.StringFlag{
		Name:  "miner.odferbase",
	...
	MinerRecommitIntervalFlag = cli.DurationFlag{
		...
		Value: odf.DefaultConfig.Miner.Recommit,
	}
	...
	RPCGlobalGasCapFlag = cli.Uint64Flag{
		Name:  "rpc.gascap",
		Usage: "Sets a cap on gas that can be used in odf_call/estimateGas (0=infinite)",
		Value: odf.DefaultConfig.RPCGasCap,
	}
	RPCGlobalTxFeeCapFlag = cli.Float64Flag{
		Name:  "rpc.txfeecap",
		Usage: "Sets a cap on transaction fee (in odfer) that can be sent via the RPC APIs (0 = no cap)",
		Value: odf.DefaultConfig.RPCTxFeeCap,
	}
	// Logging and debug settings
	EthStatsURLFlag = cli.StringFlag{
		Name:  "odfstats",
		Usage: "Reporting URL of a odfstats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: odf.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpo.percentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: odf.DefaultConfig.GPO.Percentile,
	}
	GpoMaxGasPriceFlag = cli.Int64Flag{
		Name:  "gpo.maxprice",
		Usage: "Maximum gas price will be recommended by gpo",
		Value: odf.DefaultConfig.GPO.MaxPrice.Int64(),
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		if ctx.GlobalBool(LegacyTestnetFlag.Name) || ctx.GlobalBool(RopstenFlag.Name) {
			// Maintain compatibility with older Godf configurations storing the
...
// modfod returns nil and an emphemeral key is to be generated.
...
func setLes(ctx *cli.Context, cfg *odf.Config) {
	...
	if cfg.UltraLightFraction <= 0 && cfg.UltraLightFraction > 100 {
		log.Error("Ultra light fraction is invalid", "had", cfg.UltraLightFraction, "updated", odf.DefaultConfig.UltraLightFraction)
		cfg.UltraLightFraction = odf.DefaultConfig.UltraLightFraction
	}
...
// for Godf and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `godf account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// setEtherbase retrieves the odferbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setEtherbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *odf.Config) {
	// Extract the current odferbase, new flag overriding legacy one
	var odferbase string
	if ctx.GlobalIsSet(LegacyMinerEtherbaseFlag.Name) {
		odferbase = ctx.GlobalString(LegacyMinerEtherbaseFlag.Name)
		log.Warn("The flag --odferbase is deprecated and will be removed in the future, please use --miner.odferbase")

	}
	if ctx.GlobalIsSet(MinerEtherbaseFlag.Name) {
		odferbase = ctx.GlobalString(MinerEtherbaseFlag.Name)
	}
	// Convert the odferbase into an address and configure it
	if odferbase != "" {
		if ks != nil {
			account, err := MakeAddress(ks, odferbase)
			if err != nil {
				Fatalf("Invalid miner odferbase: %v", err)
			}
			cfg.Miner.Etherbase = account.Address
		} else {
			Fatalf("No odferbase configured")
		}
	}
}
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	odfPeers := cfg.MaxPeers - lightPeers
	if lightClient {
		odfPeers = 0
	}
	log.Info("Maximum peer count", "ETH", odfPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func setGPO(ctx *cli.Context, cfg *gasprice.Config, light bool) {
	...
	if light {
		cfg.Blocks = odf.DefaultLightGPOConfig.Blocks
		cfg.Percentile = odf.DefaultLightGPOConfig.Percentile
	}
...
func setEthash(ctx *cli.Context, cfg *odf.Config) {
...
func setWhitelist(ctx *cli.Context, cfg *odf.Config) {
...
func SetShhConfig(ctx *cli.Context, stack *node.Node) {
	if ctx.GlobalIsSet(WhisperEnabledFlag.Name) ||
		...
		log.Warn("Whisper support has been deprecated and the code has been moved to github.com/odf/whisper")
	}
}

// SetEthConfig applies odf-related command line flags to the config.
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *odf.Config) {
...
// no URLs are set.
func SetDNSDiscoveryDefaults(cfg *odf.Config, genesis common.Hash) {
...
func RegisterEthService(stack *node.Node, cfg *odf.Config) odfapi.Backend {
	if cfg.SyncMode == downloader.LightSync {
...
// the given node.
func RegisterEthStatsService(stack *node.Node, backend odfapi.Backend, url string) {
	if err := odfstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Ethereum Stats service: %v", err)
	}
}

// RegisterGraphQLService is a utility function to construct a new service and register it against a node.
func RegisterGraphQLService(stack *node.Node, backend odfapi.Backend, cfg node.Config) {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "godf.", tagsMap)
		}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) odfdb.Database {
	var (
		cache   = ctx.GlobalInt(CacheFlag.Name) * ctx.GlobalInt(CacheDatabaseFlag.Name) / 100
		handles = makeDatabaseHandles()

		err     error
		chainDb odfdb.Database
	)
...
func MakeChain(ctx *cli.Context, stack *node.Node, readOnly bool) (chain *core.BlockChain, chainDb odfdb.Database) {
	var err error
	chainDb = MakeChainDatabase(ctx, stack)
	...
	if config.Clique != nil {
		...
		if !ctx.GlobalBool(FakePoWFlag.Name) {
			engine = odfash.New(odfash.Config{
				CacheDir:         stack.ResolvePath(odf.DefaultConfig.Ethash.CacheDir),
				CachesInMem:      odf.DefaultConfig.Ethash.CachesInMem,
				CachesOnDisk:     odf.DefaultConfig.Ethash.CachesOnDisk,
				CachesLockMmap:   odf.DefaultConfig.Ethash.CachesLockMmap,
				DatasetDir:       stack.ResolvePath(odf.DefaultConfig.Ethash.DatasetDir),
				DatasetsInMem:    odf.DefaultConfig.Ethash.DatasetsInMem,
				DatasetsOnDisk:   odf.DefaultConfig.Ethash.DatasetsOnDisk,
				DatasetsLockMmap: odf.DefaultConfig.Ethash.DatasetsLockMmap,
			}, nil, false)
		}
	}
	if gcmode := ctx.GlobalString(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
	...
	cache := &core.CacheConfig{
		TrieCleanLimit:      odf.DefaultConfig.TrieCleanCache,
		TrieCleanNoPrefetch: ctx.GlobalBool(CacheNoPrefetchFlag.Name),
		TrieDirtyLimit:      odf.DefaultConfig.TrieDirtyCache,
		TrieDirtyDisabled:   ctx.GlobalString(GCModeFlag.Name) == "archive",
		TrieTimeLimit:       odf.DefaultConfig.TrieTimeout,
		SnapshotLimit:       odf.DefaultConfig.SnapshotCache,
	}
	if !ctx.GlobalIsSet(SnapshotFlag.Name) {
...
// new format.
//
// e.g. godf account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this modfod with:
//
// godf --keystore /tmp/mykeystore --lightkdf account new
//
