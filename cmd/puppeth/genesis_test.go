...
// Tests the go-odf to Alodf chainspec conversion for the Stureby testnet.
func TestAlodfSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_godf.json")
	if err != nil {
	...
	spec, err := newAlodfGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_alodf.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &alodfGenesisSpec{}
...
// Tests the go-odf to Parity chainspec conversion for the Stureby testnet.
