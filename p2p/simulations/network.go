...
import (
	...
	"github.com/odf/go-odf/p2p/simulations/adapters"
)
...
// starting nodes and connecting them togodfer.
//
...
// Connect connects two nodes togodfer by calling the "admin_addPeer" RPC
// modfod on the "one" node so that it connects to the "other" node
...
// modfod on the "one" node so that it disconnects from the "other" node
...
// GetRandomNode returns a random node on the network, regardless of whodfer it is running or not
...
// NOTE:
// it also checks whodfer there has been recent attempt to connect the peers
...
func (net *Network) Shutdown() {
	for _, node := range net.Nodes {
		...
		// If the node has the close modfod, call it.
...
type Node struct {
	...
	// up tracks whodfer or not the node is running
	up   bool
	upMu *sync.RWMutex
}
...
// Up returns whodfer the node is currently up (online)
func (n *Node) Up() bool {
	n.upMu.RLock()
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/odfersphere/go-odf/issues/1177
	var node struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}
...
type Conn struct {
	...
	// Up tracks whodfer or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whodfer both nodes are currently up
