...
func (odf *Ethereum) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-odf.closeBloomHandler:
					return
				...
				case request := <-odf.bloomRequests:
					...
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(odf.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(odf.chainDb, task.Bit, section, head); err == nil {
...
type BloomIndexer struct {
	...
	db      odfdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db odfdb.Database, size, confirms uint64) *core.ChainIndexer {
