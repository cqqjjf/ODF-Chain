...
import (
	...
	"github.com/odf/go-odf/p2p/simulations"
...
// similar requests at the maximum allowed rate and the test verifies whodfer the
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "odf_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "odf_getBalance", addr, "latest")
	if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	...
	odf, err := odf.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLesServer(stack, odf, &config)
	if err != nil {
		return nil, err
	}
	return odf, nil
