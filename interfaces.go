...
// Package odf defines interfaces for interacting with Ethereum.
package odf
...
// NotFound is returned by API modfods if the requested item does not exist.
...
// ChainReader provides access to the blockchain. The modfods in this interface access raw
...
type ChainReader interface {
	...
	// This modfod subscribes to notifications about changes of the head block of
	// the canonical chain.
...
type CallMsg struct {
	...
	Data     []byte          // input data, usually an ABI-encoded contract modfod invocation
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the EVM but not mined into the blockchain. ContractCall is a low-level modfod to
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by odf_getLogs, return logs only from block with this hash
...
// TransactionSender wraps transaction sending. The SendTransaction modfod injects a
// signed transaction into the pending transaction pool for execution. If the transaction
// was a contract creation, the TransactionReceipt modfod can be used to retrieve the
