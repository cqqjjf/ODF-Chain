...
import (
	...
	"github.com/odf/go-odf/accounts/abi/bind/backends"
	"github.com/odf/go-odf/common"
	"github.com/odf/go-odf/common/mclock"
	"github.com/odf/go-odf/consensus/odfash"
	"github.com/odf/go-odf/contracts/checkpointoracle/contract"
	...
	"github.com/odf/go-odf/les/checkpointoracle"
	"github.com/odf/go-odf/les/flowcontrol"
	"github.com/odf/go-odf/light"
	"github.com/odf/go-odf/p2p"
	"github.com/odf/go-odf/p2p/enode"
	"github.com/odf/go-odf/p2p/nodestate"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some odfer to user1
		...
		case 1:
			...
			// bankUser transfers more odfer to user1
		...
		case 2:
			// bankUser transfer some odfer to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db odfdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
	var indexers [3]*core.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms, disablePruning)
	indexers[1] = odf.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db odfdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
	...
	client := &LightEthereum{
		lesCommons: lesCommons{
			...
			config:      &odf.Config{LightPeers: 100, NetworkId: NetworkId},
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db odfdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
