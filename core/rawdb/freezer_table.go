...
// preopen opens all files that the freezer will need. This modfod should be called from an init-context,
...
func (t *freezerTable) truncate(items uint64) error {
	...
	// Somodfing's out of sync, truncate the table's offset index
	log := t.logger.Debug
	if existing > items+1 {
...
//
// Note, this modfod will *not* flush any data to disk so be sure to explicitly
...
// has returns an indicator whodfer the specified number data
