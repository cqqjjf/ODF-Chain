...
type TestCmd struct {
	// For total convenience, all testing modfods are available.
	*testing.T
...
// reexec init function for that name (e.g. "godf-test" in cmd/godf/run_test.go)
...
// This modfod can also be called from an expect template, e.g.:
//
//     godf.expect(`Passphrase: {{.InputLine "password"}}`)
