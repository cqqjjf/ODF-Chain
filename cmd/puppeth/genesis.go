...
import (
	...
	math2 "github.com/odf/go-odf/common/math"
...
// alodfGenesisSpec represents the genesis specification format used by the
// C++ Ethereum implementation.
type alodfGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*alodfGenesisSpecAccount `json:"accounts"`
}

// alodfGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type alodfGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance,omitempty"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *alodfGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// alodfGenesisSpecBuiltin is the precompiled contract definition.
type alodfGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock *hexutil.Big                   `json:"startingBlock,omitempty"`
	Linear        *alodfGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type alodfGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlodfGenesisSpec converts a go-odf genesis block into a Alodf-specific
// chain specification format.
func newAlodfGenesisSpec(network string, genesis *core.Genesis) (*alodfGenesisSpec, error) {
	// Only odfash is currently supported between go-odf and alodf
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Alodf format
	spec := &alodfGenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	// config but alodf will sync with ETC clients up until the actual dao hard
	// fork block.
	...
	spec.Params.BlockReward = (*hexutil.Big)(odfash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &alodfGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &alodfGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &alodfGenesisSpecBuiltin{Name: "sha256",
		Linear: &alodfGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &alodfGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &alodfGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &alodfGenesisSpecBuiltin{Name: "identity",
		Linear: &alodfGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &alodfGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
		spec.setPrecompile(6, &alodfGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alodfGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &alodfGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
			Linear:        &alodfGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &alodfGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock)})
	}
	if genesis.Config.IstanbulBlock != nil {
		...
		spec.setPrecompile(6, &alodfGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_add",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Alodf hardcoded the gas policy
		spec.setPrecompile(7, &alodfGenesisSpecBuiltin{
			Name:          "alt_bn128_G1_mul",
			StartingBlock: (*hexutil.Big)(genesis.Config.ByzantiumBlock),
		}) // Alodf hardcoded the gas policy
		spec.setPrecompile(9, &alodfGenesisSpecBuiltin{
...
func (spec *alodfGenesisSpec) setPrecompile(address byte, data *alodfGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alodfGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &alodfGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *alodfGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alodfGenesisSpecAccount)
	}
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-odf/pull/11039,
...
// newParityChainSpec converts a go-odf genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only odfash is currently supported between go-odf and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(odfash.ByzantiumBlockReward)
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(odfash.ConstantinopleBlockReward)
...
// newPyEthereumGenesisSpec converts a go-odf genesis block into a Parity specific
// chain specification format.
func newPyEthereumGenesisSpec(network string, genesis *core.Genesis) (*pyEthereumGenesisSpec, error) {
	// Only odfash is currently supported between go-odf and pyodf
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
