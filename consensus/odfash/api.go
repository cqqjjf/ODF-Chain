...
var errEthashStopped = errors.New("odfash stopped")

// API exposes odfash related modfods for the RPC interface.
type API struct {
	odfash *Ethash
}
...
func (api *API) GetWork() ([4]string, error) {
	...
	select {
	case api.odfash.remote.fetchWorkCh <- &sealWork{errc: errc, res: workCh}:
	case <-api.odfash.remote.exitCh:
...
func (api *API) SubmitWork(nonce types.BlockNonce, hash, digest common.Hash) bool {
	...
	select {
	case api.odfash.remote.submitWorkCh <- &mineResult{
		nonce:     nonce,
		mixDigest: digest,
		hash:      hash,
		errc:      errc,
...
func (api *API) SubmitHashRate(rate hexutil.Uint64, id common.Hash) bool {
	...
	select {
	case api.odfash.remote.submitRateCh <- &hashrate{done: done, rate: uint64(rate), id: id}:
	case <-api.odfash.remote.exitCh:
		return false
	}
...
func (api *API) GetHashrate() uint64 {
	return uint64(api.odfash.Hashrate())
}
