...
import (
	...
	"github.com/odf/go-odf/internal/jsre/deps"
	"github.com/odf/go-odf/internal/web3ext"
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"odf": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
// initPersonal redirects account-related API modfods through the bridge.
//
// If the console is in interactive mode and the 'personal' API is available, override
// the openWallet, unlockAccount, newAccount and sign modfods since these require user
// interaction. The original web3 callbacks are stored in 'jodf'. These will be called
...
func (c *Console) initPersonal(vm *goja.Runtime, bridge *bridge) {
	...
	jodf := vm.NewObject()
	vm.Set("jodf", jodf)
	jodf.Set("openWallet", personal.Get("openWallet"))
	jodf.Set("unlockAccount", personal.Get("unlockAccount"))
	jodf.Set("newAccount", personal.Get("newAccount"))
	jodf.Set("sign", personal.Get("sign"))
...
// consoleOutput is an override for the console.log and console.error modfods to
...
// input prompter to provide hints to the user about the modfods available.
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines odf.getBalance(odf.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
		// Skip all modfods and namespaces (i.e. including the dot)
...
// Welcome show summary of current Godf instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Godf JavaScript console!\n\n"

	// Print some generic Godf metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + odf.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + odf.blockNumber + " (" + new Date(1000 * odf.getBlock(odf.blockNumber).timestamp) + ")\n";
		try {
