...
// Package promodfeus exposes go-metrics into a Promodfeus format.
package promodfeus
...
// Handler returns an HTTP handler which dump metrics in Promodfeus format.
func Handler(reg metrics.Registry) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Aggregate all the metris into a Promodfeus collector
		c := newCollector()
		...
		for _, name := range names {
			...
			default:
				log.Warn("Unknown Promodfeus metric type", "type", fmt.Sprintf("%T", i))
			}
		}
