...
FROM odf/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--odfstats", "{{.Ethstats}}", "--odfport", "{{.EthPort}}",     \
...
// checkFaucet does a health-check against a faucet server to verify whodfer
