...
import (
	...
	"github.com/odf/go-odf/tests"
)

// To generate a new callTracer test, copy paste the makeTest modfod below into
// a Godf console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = odf.getBlock(odf.getTransaction(tx).blockHash);
  var genesis = odf.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.odf.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  odf.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
