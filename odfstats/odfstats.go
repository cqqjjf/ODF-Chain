...
// Package odfstats implements the network stats reporting service.
package odfstats
...
// backend encompasses the bare-minimum functionality needed for odfstats reporting
...
//   Applications are responsible for ensuring that no more than one goroutine calls the write modfods
//     - NextWriter, SetWriteDeadline, WriteMessage, WriteJSON, EnableWriteCompression, SetCompressionLevel
//   concurrently and that no more than one goroutine calls the read modfods
//     - NextReader, SetReadDeadline, ReadMessage, ReadJSON, SetPongHandler, SetPingHandler
//   concurrently.
//   The Close and WriteControl modfods can be called concurrently with all other modfods.
...
// WriteJSON wraps corresponding modfod on the websocket but is safe for concurrent calling
...
// ReadJSON wraps corresponding modfod on the websocket but is safe for concurrent calling
...
// Close wraps corresponding modfod on the websocket but is safe for concurrent calling
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(odfstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["odf"]; info != nil {
		network = fmt.Sprintf("%d", info.(*odf.NodeInfo).Network)
		protocol = fmt.Sprintf("odf/%d", odf.ProtocolVersions[0])
	} else {
...
// server. Use the individual modfods for reporting subscribed events.
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the odfstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to odfstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to odfstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to odfstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to odfstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to odfstats")
