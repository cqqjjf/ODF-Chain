...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.odfstats == "" {
		log.Error("No odfstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.odfashdir == "" {
			fmt.Printf("Where should the odfash mining DAGs be stored on the remote machine?\n")
			infos.odfashdir = w.readString()
		} else {
			fmt.Printf("Where should the odfash mining DAGs be stored on the remote machine? (default = %s)\n", infos.odfashdir)
			infos.odfashdir = w.readDefaultString(infos.odfashdir)
		}
	}
	...
	if infos.odfstats == "" {
		...
		infos.odfstats = w.readString() + ":" + w.conf.odfstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.odfstats)
		infos.odfstats = w.readDefaultString(infos.odfstats) + ":" + w.conf.odfstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an odferbase to mine against
			fmt.Println()
			if infos.odferbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.odferbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.odferbase)
				infos.odferbase = w.readDefaultAddress(common.HexToAddress(infos.odferbase)).Hex()
			}
