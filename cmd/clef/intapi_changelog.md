...
- The `message` field was renamed to `messages` in all data signing request modfods to better reflect that it's a list, not a value.
- The `storage.Put` and `storage.Get` modfods in the rule execution engine were lower-cased to `storage.put` and `storage.get` to be consistent with JavaScript call conventions.
...
Changed the namespace format to adhere to the legacy odf format: `name_modfodName`. Changes:
...
* Bidirectional communication implemented, so the UI can query `clef` via the stdin/stdout RPC channel. Modfods implemented are:
  - `clef_listWallets`
  - `clef_listAccounts`
  - `clef_listWallets`
  - `clef_deriveAccount`
...
* Add `OnInputRequired(info UserInputRequest)` to internal API. This modfod is used when Clef needs user input, e.g. passwords.
...
* Add `OnStartup` modfod, to provide the UI with information about what API version
