...
// godf is the official command-line client for Ethereum.
package main
...
import (
	...
	"github.com/odf/go-odf/internal/debug"
...
const (
	clientIdentifier = "godf" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-odf command line interface")
...
func init() {
	// Initialize the CLI app and start Godf
	app.Action = godf
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2020 The go-odf Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.LegacyTestnetFlag.Name):
		log.Info("Starting Godf on Ropsten testnet...")
	...
	case ctx.GlobalIsSet(utils.RinkebyFlag.Name):
		log.Info("Starting Godf on Rinkeby testnet...")

	case ctx.GlobalIsSet(utils.GoerliFlag.Name):
		log.Info("Starting Godf on G"orli testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Godf in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Godf on Ethereum mainnet...")
	}
...
// godf is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func godf(ctx *cli.Context) error {
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, backend odfapi.Backend) {
	...
	// Create a client to interact with local godf node.
	rpcClient, err := stack.Attach()
	if err != nil {
		utils.Fatalf("Failed to attach to self: %v", err)
	}
	odfClient := odfclient.NewClient(rpcClient)
	...
	go func() {
		...
		for event := range events {
			...
			case accounts.WalletOpened:
				...
				event.Wallet.SelfDerive(derivationPaths, odfClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		odfBackend, ok := backend.(*odf.EthAPIBackend)
		if !ok {
		...
		odfBackend.TxPool().SetGasPrice(gasprice)
		// start mining
		...
		if err := odfBackend.StartMining(threads); err != nil {
