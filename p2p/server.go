...
import (
	...
	"github.com/odf/go-odf/p2p/discover"
...
type Config struct {
	...
	// DiscoveryV5 specifies whodfer the new topic-discovery based V5 discovery
...
//
// This modfod blocks until all protocols have exited and the peer is removed. Do not use
...
func (srv *Server) setupDiscovery() error {
	...
	if srv.NAT != nil {
		if !realaddr.IP.IsLoopback() {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "odf discovery")
				srv.loopWG.Done()
			}()
		}
	}
...
func (srv *Server) setupListening() error {
	...
	if tcp, ok := listener.Addr().(*net.TCPAddr); ok {
		...
		if !tcp.IP.IsLoopback() && srv.NAT != nil {
			srv.loopWG.Add(1)
			go func() {
				nat.Map(srv.NAT, srv.quit, "tcp", tcp.Port, tcp.Port, "odf p2p")
				srv.loopWG.Done()
			}()
		}
	}
