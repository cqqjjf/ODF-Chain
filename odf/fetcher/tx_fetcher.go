...
const (
	...
	//   - In odf protocol, the softResponseLimit is 2MB. Nowadays according to
...
var (
	txAnnounceInMeter          = metrics.NewRegisteredMeter("odf/fetcher/transaction/announces/in", nil)
	txAnnounceKnownMeter       = metrics.NewRegisteredMeter("odf/fetcher/transaction/announces/known", nil)
	txAnnounceUnderpricedMeter = metrics.NewRegisteredMeter("odf/fetcher/transaction/announces/underpriced", nil)
	txAnnounceDOSMeter         = metrics.NewRegisteredMeter("odf/fetcher/transaction/announces/dos", nil)

	txBroadcastInMeter          = metrics.NewRegisteredMeter("odf/fetcher/transaction/broadcasts/in", nil)
	txBroadcastKnownMeter       = metrics.NewRegisteredMeter("odf/fetcher/transaction/broadcasts/known", nil)
	txBroadcastUnderpricedMeter = metrics.NewRegisteredMeter("odf/fetcher/transaction/broadcasts/underpriced", nil)
	txBroadcastOtherRejectMeter = metrics.NewRegisteredMeter("odf/fetcher/transaction/broadcasts/otherreject", nil)

	txRequestOutMeter     = metrics.NewRegisteredMeter("odf/fetcher/transaction/request/out", nil)
	txRequestFailMeter    = metrics.NewRegisteredMeter("odf/fetcher/transaction/request/fail", nil)
	txRequestDoneMeter    = metrics.NewRegisteredMeter("odf/fetcher/transaction/request/done", nil)
	txRequestTimeoutMeter = metrics.NewRegisteredMeter("odf/fetcher/transaction/request/timeout", nil)

	txReplyInMeter          = metrics.NewRegisteredMeter("odf/fetcher/transaction/replies/in", nil)
	txReplyKnownMeter       = metrics.NewRegisteredMeter("odf/fetcher/transaction/replies/known", nil)
	txReplyUnderpricedMeter = metrics.NewRegisteredMeter("odf/fetcher/transaction/replies/underpriced", nil)
	txReplyOtherRejectMeter = metrics.NewRegisteredMeter("odf/fetcher/transaction/replies/otherreject", nil)

	txFetcherWaitingPeers   = metrics.NewRegisteredGauge("odf/fetcher/transaction/waiting/peers", nil)
	txFetcherWaitingHashes  = metrics.NewRegisteredGauge("odf/fetcher/transaction/waiting/hashes", nil)
	txFetcherQueueingPeers  = metrics.NewRegisteredGauge("odf/fetcher/transaction/queueing/peers", nil)
	txFetcherQueueingHashes = metrics.NewRegisteredGauge("odf/fetcher/transaction/queueing/hashes", nil)
	txFetcherFetchingPeers  = metrics.NewRegisteredGauge("odf/fetcher/transaction/fetching/peers", nil)
	txFetcherFetchingHashes = metrics.NewRegisteredGauge("odf/fetcher/transaction/fetching/hashes", nil)
)
...
type txDelivery struct {
	...
	direct bool          // Whodfer this is a direct reply or a broadcast
}
...
// NewTxFetcherForTests is a testing modfod to mock out the realtime clock with
...
// and the fetcher. This modfod may be called by both transaction broadcasts and
...
func (f *TxFetcher) loop() {
	...
	for {
		...
		case ann := <-f.notify:
			...
			// the probability of somodfing arriving between this call and the pre-
			...
			// If this peer is new and announced somodfing already queued, maybe
		...
		case <-timeoutTrigger:
			...
			// No idea if we scheduled somodfing or not, trigger the timer if needed
		...
		case delivery := <-f.cleanup:
			...
			if delivery.direct {
				...
				// Make sure somodfing was pending, nuke it
				...
				// Somodfing was delivered, try to rechedule requests
		...
		// Loop did somodfing, ping the step notifier if needed (tests)
		if f.step != nil {
			f.step <- struct{}{}
		}
	}
...
//
// The modfod has a granularity of 'gatherSlack', since there's not much point in
...
// a few ms earlier.
//
// This modfod is a bit "flaky" "by design". In theory the timeout timer only ever
...
// be set again since len(request) > 0 => somodfing's running.
func (f *TxFetcher) rescheduleTimeout(timer *mclock.Timer, trigger chan struct{}) {
	...
	for _, req := range f.requests {
		// If this request already timed out, skip it altogodfer
		if req.hashes == nil {
			continue
		}
...
// rotateStrings rotates the contents of a slice by n steps. This modfod is only
...
// sortHashes sorts a slice of hashes. This modfod is only used in tests in order
...
// rotateHashes rotates the contents of a slice by n steps. This modfod is only
// used in tests to simulate random map iteration but keep it deterministic.
