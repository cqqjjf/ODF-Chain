...
import (
	...
	"github.com/odf/go-odf/internal/testlog"
	"github.com/odf/go-odf/log"
)

// Tests whodfer remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom odfash engine.
	odfash := NewTester([]string{server.URL}, false)
	defer odfash.Close()
	...
	odfash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := odfash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom odfash engine.
	odfash := NewTester([]string{server.URL}, false)
	odfash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer odfash.Close()
	...
	for i := 0; i < cap(sink); i++ {
		...
		block := types.NewBlockWithHeader(header)
		odfash.Seal(nil, block, results, nil)
	}
...
// Tests whodfer stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	odfash := NewTester(nil, true)
	defer odfash.Close()
	api := &API{odfash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			odfash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, odfash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch odfash result timeout", id+1)
		}
	}
}
