...
creating a collection of devp2p nodes, connecting them togodfer to form a
...
if a node is created without an explicit service), exposes modfods for
...
connecting two nodes togodfer).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somodfing happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` modfod can be called
with a `Step` which has the following fields:

* `Action` - a function which performs some action in the network

* `Expect` - an expectation function which returns whodfer or not a
...
On return, the `Simulation.Run` modfod returns a `StepResult` which can be used
...
p2psim node rpc <node> <modfod> [<args>] [--subscribe]
```
