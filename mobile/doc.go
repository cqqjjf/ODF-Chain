...
// Package godf contains the simplified mobile APIs to go-odf.
//
// The scope of this package is *not* to allow writing a custom Ethereum client
// with pieces plucked from go-odf, rather to allow writing native dapps on
...
// from the go-odf repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-odf version.
// This is needed because gomobile cannot expose named types for now.
//
// Whenever a modfod argument or a return type is a custom struct, the pointer
...
// a go slice with the modfods `Size`, `Get` and `Set`. Further slice operations
...
//
// If a modfod has multiple return values (e.g. some return + an error), those
