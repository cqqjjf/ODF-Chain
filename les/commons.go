...
func (c *lesCommons) setupOracle(node *node.Node, genesis common.Hash, odfconfig *odf.Config) *checkpointoracle.CheckpointOracle {
	config := odfconfig.CheckpointOracle
	if config == nil {
	...
	client := odfclient.NewClient(rpcClient)
	oracle.Start(client)
