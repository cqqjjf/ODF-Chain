...
import (
	...
	"github.com/odf/go-odf/signer/storage"
)
...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, modfodSelector *string) (*odfapi.SignTransactionResult, error)
	...
	SignGnosisSafeTx(ctx context.Context, signerAddress common.MixedcaseAddress, gnosisTx GnosisSafeTx, modfodSelector *string) (*GnosisSafeTx, error)
}

// UIClientAPI specifies what modfod a UI needs to implement to be able to be used as a
// UI for the signer
type UIClientAPI interface {
	...
	// This modfod can be used by a UI to keep track of e.g. how much has been sent to a particular recipient.
	OnApprovedTx(tx odfapi.SignTransactionResult)
...
// Validator defines the modfods required to validate a transaction against some
// sanity defaults as well as any underlying 4byte modfod database.
//
...
// newAccount is the internal modfod to create a new account. It should be used
...
func (api *SignerAPI) lookupOrQueryPassword(address common.Address, title, prompt string) (string, error) {
	...
	if err != nil {
		...
		// which could leak the password if it was malformed json or somodfing
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, modfodSelector *string) (*odfapi.SignTransactionResult, error) {
	var (
		err    error
		result SignTxResponse
	)
	msgs, err := api.validator.ValidateTransaction(modfodSelector, &args)
	if err != nil {
		return nil, err
	}
	...
	response := odfapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
func (api *SignerAPI) SignGnosisSafeTx(ctx context.Context, signerAddress common.MixedcaseAddress, gnosisTx GnosisSafeTx, modfodSelector *string) (*GnosisSafeTx, error) {
...
// Returns the external api version. This modfod does not require user acceptance. Available modfods are
