...
// Tests that initializing Godf with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	for i, tt := range customGenesisTests {
		...
		runGodf(t, "--nousb", "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		godf := runGodf(t, "--nousb",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		godf.ExpectRegexp(tt.result)
		godf.ExpectExit()
	}
}
