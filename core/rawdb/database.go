...
import (
	...
	"github.com/odf/go-odf/odfdb/leveldb"
...
// Freeze is a helper modfod used for external testing to trigger and block until
...
func NewDatabase(db odfdb.KeyValueStore) odfdb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
// storage.
func NewDatabaseWithFreezer(db odfdb.KeyValueStore, freezer string, namespace string) (odfdb.Database, error) {
	...
	// validate in this modfod. If, however, the genesis hash is not nil, compare
	// it to the freezer content.
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains somodfing, ensure that the genesis blocks
...
func NewMemoryDatabase() odfdb.Database {
...
func NewMemoryDatabaseWithCap(size int) odfdb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (odfdb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string) (odfdb.Database, error) {
...
func InspectDatabase(db odfdb.Database) error {
