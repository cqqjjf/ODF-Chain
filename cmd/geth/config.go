...
type godfConfig struct {
	Eth      odf.Config
	Shh      whisperDeprecatedConfig
	Node     node.Config
	Ethstats odfstatsConfig
}

func loadConfig(file string, cfg *godfConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "odf")
	cfg.WSModules = append(cfg.WSModules, "odf")
	cfg.IPCPath = "godf.ipc"
	return cfg
}

// makeConfigNode loads godf configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, godfConfig) {
	// Load defaults.
	cfg := godfConfig{
		Eth:  odf.DefaultConfig,
	...
	if file := ctx.GlobalString(configFileFlag.Name); file != "" {
		...
		if cfg.Shh != (whisperDeprecatedConfig{}) {
			log.Warn("Deprecated whisper config detected. Whisper has been moved to github.com/odf/whisper")
		}
	}
...
func checkWhisper(ctx *cli.Context) {
	for _, flag := range whisperFlags {
		if ctx.GlobalIsSet(flag.GetName()) {
			log.Warn("deprecated whisper flag detected. Whisper has been moved to github.com/odf/whisper")
		}
	}
}

// makeFullNode loads godf configuration and creates the Ethereum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, odfapi.Backend) {
