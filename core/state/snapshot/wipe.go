...
func wipeSnapshot(db odfdb.KeyValueStore, full bool) chan struct{} {
...
func wipeContent(db odfdb.KeyValueStore) error {
...
func wipeKeyRange(db odfdb.KeyValueStore, kind string, prefix []byte, keylen int) error {
	// Batch deletions togodfer to avoid holding an iterator for too long
	var (
		batch = db.NewBatch()
		items int
	)
