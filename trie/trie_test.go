...
func init() {
	spew.Config.Indent = "    "
	spew.Config.DisableModfods = false
}
...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"somodfingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
...
func (s *spongeDb) NewBatch() odfdb.Batch                    { return &spongeBatch{s} }
...
func (s *spongeDb) NewIterator(prefix []byte, start []byte) odfdb.Iterator { panic("implement me") }
...
func (b *spongeBatch) Replay(w odfdb.KeyValueWriter) error { return nil }
...
// to check whodfer changes to the trie modifies the write order or data in any way.
