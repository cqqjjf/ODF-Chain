...
import (
	...
	lps "github.com/odf/go-odf/les/lespay/server"
	"github.com/odf/go-odf/les/utils"
...
type peerCommons struct {
	*p2p.Peer
	...
	frozen       uint32    // Flag whodfer the peer is frozen.
...
// canQueue returns an indicator whodfer the peer can queue an operation.
...
func (p *peerCommons) Info() *odf.PeerInfo {
	return &odf.PeerInfo{
		Version:    p.version,
		Difficulty: p.Td(),
...
type serverPeer struct {
	...
	// Status fields
	trusted                 bool   // The flag whodfer the server is selected as trusted server.
	onlyAnnounce            bool   // The flag whodfer the server sends announcement only.
	...
	// Test callback hooks
	hasBlockHook func(common.Hash, uint64, bool) bool // Used to determine whodfer the server has the specified block.
}
	...
	return p.handshake(td, head, headNum, genesis, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local odf node is running in archive mode, advertise ourselves we have
