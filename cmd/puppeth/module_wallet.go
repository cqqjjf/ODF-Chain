...
FROM puppodf/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'godf --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec godf --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --odfstats \'{{.Ethstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppodfNetworkID/{{.NetworkID}}/g' dist/js/odferwallet-master.js && \
	sed -i 's/PuppodfNetwork/{{.Network}}/g'     dist/js/odferwallet-master.js && \
	sed -i 's/PuppodfDenom/{{.Denom}}/g'         dist/js/odferwallet-master.js && \
	sed -i 's/PuppodfHost/{{.Host}}/g'           dist/js/odferwallet-master.js && \
	sed -i 's/PuppodfRPCPort/{{.RPCPort}}/g'     dist/js/odferwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whodfer
// it's running, and if yes, whodfer it's responsive.
