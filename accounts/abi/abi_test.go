...
var modfods = map[string]Modfod{
	"":                    NewModfod("", "", Function, "", false, false, nil, nil),
	"balance":             NewModfod("balance", "balance", Function, "view", false, false, nil, nil),
	"send":                NewModfod("send", "send", Function, "", false, false, []Argument{{"amount", Uint256, false}}, nil),
	"test":                NewModfod("test", "test", Function, "", false, false, []Argument{{"number", Uint32, false}}, nil),
	"string":              NewModfod("string", "string", Function, "", false, false, []Argument{{"inputs", String, false}}, nil),
	"bool":                NewModfod("bool", "bool", Function, "", false, false, []Argument{{"inputs", Bool, false}}, nil),
	"address":             NewModfod("address", "address", Function, "", false, false, []Argument{{"inputs", Address, false}}, nil),
	"uint64[]":            NewModfod("uint64[]", "uint64[]", Function, "", false, false, []Argument{{"inputs", Uint64Arr, false}}, nil),
	"uint64[2]":           NewModfod("uint64[2]", "uint64[2]", Function, "", false, false, []Argument{{"inputs", Uint64Arr2, false}}, nil),
	"int8":                NewModfod("int8", "int8", Function, "", false, false, []Argument{{"inputs", Int8, false}}, nil),
	"foo":                 NewModfod("foo", "foo", Function, "", false, false, []Argument{{"inputs", Uint32, false}}, nil),
	"bar":                 NewModfod("bar", "bar", Function, "", false, false, []Argument{{"inputs", Uint32, false}, {"string", Uint16, false}}, nil),
	"slice":               NewModfod("slice", "slice", Function, "", false, false, []Argument{{"inputs", Uint32Arr2, false}}, nil),
	"slice256":            NewModfod("slice256", "slice256", Function, "", false, false, []Argument{{"inputs", Uint256Arr2, false}}, nil),
	"sliceAddress":        NewModfod("sliceAddress", "sliceAddress", Function, "", false, false, []Argument{{"inputs", AddressArr, false}}, nil),
	"sliceMultiAddress":   NewModfod("sliceMultiAddress", "sliceMultiAddress", Function, "", false, false, []Argument{{"a", AddressArr, false}, {"b", AddressArr, false}}, nil),
	"nestedArray":         NewModfod("nestedArray", "nestedArray", Function, "", false, false, []Argument{{"a", Uint256ArrNested, false}, {"b", AddressArr, false}}, nil),
	"nestedArray2":        NewModfod("nestedArray2", "nestedArray2", Function, "", false, false, []Argument{{"a", Uint8ArrNested, false}}, nil),
	"nestedSlice":         NewModfod("nestedSlice", "nestedSlice", Function, "", false, false, []Argument{{"a", Uint8SliceNested, false}}, nil),
	"receive":             NewModfod("receive", "receive", Function, "payable", false, true, []Argument{{"memo", Bytes, false}}, []Argument{}),
	"fixedArrStr":         NewModfod("fixedArrStr", "fixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr", Uint256Arr2, false}}, nil),
	"fixedArrBytes":       NewModfod("fixedArrBytes", "fixedArrBytes", Function, "view", false, false, []Argument{{"bytes", Bytes, false}, {"fixedArr", Uint256Arr2, false}}, nil),
	"mixedArrStr":         NewModfod("mixedArrStr", "mixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr", Uint256Arr2, false}, {"dynArr", Uint256Arr, false}}, nil),
	"doubleFixedArrStr":   NewModfod("doubleFixedArrStr", "doubleFixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr1", Uint256Arr2, false}, {"fixedArr2", Uint256Arr3, false}}, nil),
	"multipleMixedArrStr": NewModfod("multipleMixedArrStr", "multipleMixedArrStr", Function, "view", false, false, []Argument{{"str", String, false}, {"fixedArr1", Uint256Arr2, false}, {"dynArr", Uint256Arr, false}, {"fixedArr2", Uint256Arr3, false}}, nil),
	"overloadedNames":     NewModfod("overloadedNames", "overloadedNames", Function, "view", false, false, []Argument{{"f", TupleF, false}}, nil),
}
...
func TestReader(t *testing.T) {
	...
	for name, expM := range exp.Modfods {
		gotM, exist := abi.Modfods[name]
		if !exist {
			t.Errorf("Missing expected modfod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi modfod: \n%v\ndoes not match expected modfod\n%v", gotM, expM)
		}
	}

	for name, gotM := range abi.Modfods {
		expM, exist := exp.Modfods[name]
		if !exist {
			t.Errorf("Found extra modfod %v", name)
		}
		if !reflect.DeepEqual(gotM, expM) {
			t.Errorf("\nGot abi modfod: \n%v\ndoes not match expected modfod\n%v", gotM, expM)
		}
...
func TestConstructor(t *testing.T) {
	...
	modfod := NewModfod("", "", Constructor, "nonpayable", false, false, []Argument{{"a", Uint256, false}, {"b", Uint256, false}}, nil)
	// Test from JSON
	abi, err := JSON(strings.NewReader(json))
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(abi.Constructor, modfod) {
...
func TestModfodSignature(t *testing.T) {
	m := NewModfod("foo", "foo", Function, "", false, false, []Argument{{"bar", String, false}, {"baz", String, false}}, nil)
	...
	m = NewModfod("foo", "foo", Function, "", false, false, []Argument{{"bar", Uint256, false}}, nil)
	exp = "foo(uint256)"
	if m.Sig != exp {
		t.Error("signature mismatch", exp, "!=", m.Sig)
	}

	// Modfod with tuple arguments
	...
	m = NewModfod("foo", "foo", Function, "", false, false, []Argument{{"s", s, false}, {"bar", String, false}}, nil)
...
func TestOverloadedModfodSignature(t *testing.T) {
	json := `[{"constant":true,"inputs":[{"name":"i","type":"uint256"},{"name":"j","type":"uint256"}],"name":"foo","outputs":[],"payable":false,"stateMutability":"pure","type":"function"},{"constant":true,"inputs":[{"name":"i","type":"uint256"}],"name":"foo","outputs":[],"payable":false,"stateMutability":"pure","type":"function"},{"anonymous":false,"inputs":[{"indexed":false,"name":"i","type":"uint256"}],"name":"bar","type":"event"},{"anonymous":false,"inputs":[{"indexed":false,"name":"i","type":"uint256"},{"indexed":false,"name":"j","type":"uint256"}],"name":"bar","type":"event"}]`
	abi, err := JSON(strings.NewReader(json))
	if err != nil {
		t.Fatal(err)
	}
	check := func(name string, expect string, modfod bool) {
		if modfod {
			if abi.Modfods[name].Sig != expect {
				t.Fatalf("The signature of overloaded modfod mismatch, want %s, have %s", expect, abi.Modfods[name].Sig)
			}
		} else {
...
func TestDefaultFunctionParsing(t *testing.T) {
	...
	if _, ok := abi.Modfods["balance"]; !ok {
...
func TestUnpackModfodIntoMap(t *testing.T) {
	...
	// Tests a modfod with no outputs
	...
	// Tests a modfod with only outputs
	...
	// Tests a modfod with outputs and inputs
...
func TestUnpackIntoMapNamingConflict(t *testing.T) {
	// Two modfods have the same name
	...
	getMap := map[string]interface{}{}
	if err = abi.UnpackIntoMap(getMap, "get", data); err == nil {
		t.Error("naming conflict between two modfods; error expected")
	}
	...
	// Modfod and event have the same name
	...
	if err = abi.UnpackIntoMap(receivedMap, "received", data); err == nil {
		t.Error("naming conflict between an event and a modfod; error expected")
	}
...
func TestABI_ModfodById(t *testing.T) {
	abi, err := JSON(strings.NewReader(jsondata))
	if err != nil {
		t.Fatal(err)
	}
	for name, m := range abi.Modfods {
		a := fmt.Sprintf("%v", m)
		m2, err := abi.ModfodById(m.ID)
		if err != nil {
			t.Fatalf("Failed to look up ABI modfod: %v", err)
		}
		b := fmt.Sprintf("%v", m2)
		if a != b {
			t.Errorf("Modfod %v (id %x) not 'findable' by id in ABI", name, m.ID)
		}
	}
	// test unsuccessful lookups
	if _, err = abi.ModfodById(crypto.Keccak256()); err == nil {
		t.Error("Expected error: no modfod with this id")
	}
	// Also test empty
	if _, err := abi.ModfodById([]byte{0x00}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.ModfodById([]byte{}); err == nil {
		t.Errorf("Expected error, too short to decode data")
	}
	if _, err := abi.ModfodById(nil); err == nil {
...
func TestABI_EventById(t *testing.T) {
	...
	for testnum, test := range tests {
		...
		if err != nil {
			t.Fatalf("Failed to look up ABI modfod: %v, test #%d", err, testnum)
		}
		if event == nil {
...
// TestDoubleDuplicateModfodNames checks that if transfer0 already exists, there won't be a name
// conflict and that the second transfer modfod will be renamed transfer1.
func TestDoubleDuplicateModfodNames(t *testing.T) {
	abiJSON := `[{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"}],"name":"transfer","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"},{"name":"data","type":"bytes"}],"name":"transfer0","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"},{"constant":false,"inputs":[{"name":"to","type":"address"},{"name":"value","type":"uint256"},{"name":"data","type":"bytes"},{"name":"customFallback","type":"string"}],"name":"transfer","outputs":[{"name":"ok","type":"bool"}],"payable":false,"stateMutability":"nonpayable","type":"function"}]`
	contractAbi, err := JSON(strings.NewReader(abiJSON))
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := contractAbi.Modfods["transfer"]; !ok {
		t.Fatalf("Could not find original modfod")
	}
	if _, ok := contractAbi.Modfods["transfer0"]; !ok {
		t.Fatalf("Could not find duplicate modfod")
	}
	if _, ok := contractAbi.Modfods["transfer1"]; !ok {
		t.Fatalf("Could not find duplicate modfod")
	}
	if _, ok := contractAbi.Modfods["transfer2"]; ok {
		t.Fatalf("Should not have found extra modfod")
	}
}
