...
func (w *wizard) deployExplorer() {
	...
	// Figure which virtual-host to deploy odfstats on
	...
	fmt.Println()
	if infos.node.odfstats == "" {
		fmt.Printf("What should the explorer be called on the stats page?\n")
		infos.node.odfstats = w.readString() + ":" + w.conf.odfstats
	} else {
		fmt.Printf("What should the explorer be called on the stats page? (default = %s)\n", infos.node.odfstats)
		infos.node.odfstats = w.readDefaultString(infos.node.odfstats) + ":" + w.conf.odfstats
	}
