...
// Backend wraps all modfods required for mining.
...
type Config struct {
	...
	Notify    []string       `toml:",omitempty"` // HTTP URL list to be notified of new work packages(only useful in odfash).
	ExtraData hexutil.Bytes  `toml:",omitempty"` // Block extra data set by the miner
	GasFloor  uint64         // Target gas floor for mined blocks.
	GasCeil   uint64         // Target gas ceiling for mined blocks.
	GasPrice  *big.Int       // Minimum gas price for mining a transaction
	Recommit  time.Duration  // The time interval for miner to re-create mining work.
	Noverify  bool           // Disable remote mining solution verification(only useful in odfash).
}
...
func New(odf Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		odf:     odf,
		mux:     mux,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, odf, mux, isLocalBlock, true),
	}
	go miner.update()
...
// change between multiple modfod calls
