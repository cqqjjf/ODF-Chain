...
{{end}}
RUN \
  echo 'godf --cache 512 init /genesis.json' > godf.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.odf/keystore/ && cp /signer.json /root/.odf/keystore/' >> godf.sh && \{{end}}
	echo $'exec godf --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --odfstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.odferbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> godf.sh

ENTRYPOINT ["/bin/sh", "godf.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.odf{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.odfash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.odferbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Ethstats":  config.odfstats,
		"Etherbase": config.odferbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ethashdir":  config.odfashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Ethstats":   config.odfstats[:strings.Index(config.odfstats, ":")],
		"Etherbase":  config.odferbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":        info.odfstats,
	}
	if info.gasTarget > 0 {
		...
		if info.odferbase != "" {
			...
			report["Ethash directory"] = info.odfashdir
			report["Miner account"] = info.odferbase
		}
		if info.keyJSON != "" {
...
// whodfer it's running, and if yes, whodfer it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 godf --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.odf"],
		odfashdir:  infos.volumes["/root/.odfash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		odfstats:   infos.envvars["STATS_NAME"],
		odferbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
