...
func TestUnpack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i)+" "+test.def, func(t *testing.T) {
			//Unpack
			def := fmt.Sprintf(`[{ "name" : "modfod", "type": "function", "outputs": %s}]`, test.def)
			...
			out, err := abi.Unpack("modfod", encb)
			if err != nil {
...
func TestLocalUnpackTests(t *testing.T) {
	for i, test := range unpackTests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			err = abi.UnpackIntoInterface(outptr.Interface(), "modfod", encb)
...
type modfodMultiOutput struct {
	Int    *big.Int
	String string
}

func modfodMultiReturn(require *require.Assertions) (ABI, []byte, modfodMultiOutput) {
	const definition = `[
	{ "name" : "multi", "type": "function", "outputs": [ { "name": "Int", "type": "uint256" }, { "name": "String", "type": "string" } ] }]`
	var expected = modfodMultiOutput{big.NewInt(1), "hello"}
...
func TestModfodMultiReturn(t *testing.T) {
	...
	abi, data, expected := modfodMultiReturn(require.New(t))
	bigint := new(big.Int)
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.error == "" {
				require.Nil(err, "Should be able to unpack modfod outputs.")
...
func TestMultiReturnWithStringSlice(t *testing.T) {
	...
	ret1, ret1Exp := new([]string), []string{"odf", "go-odf"}
...
func TestUnpackTuple(t *testing.T) {
	...
	type T struct {
		X *big.Int `abi:"x"`
		Z *big.Int `abi:"y"` // Test whodfer the abi tag works.
	}
...
func TestOOMMaliciousInput(t *testing.T) {
	...
	for i, test := range oomTests {
		...
		_, err = abi.Modfods["modfod"].Outputs.UnpackValues(encb)
		if err == nil {
