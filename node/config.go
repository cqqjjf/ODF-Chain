...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of godf is "godf". If no
	...
	// registered services, instead those can use utility modfods to create/access
...
// ExtRPCEnabled returns the indicator whodfer node enables the external
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Godf", keep that.
	if name == "godf" || name == "godf-testnet" {
		name = "Godf"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "godf" instances.
var isOldGodfResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by godf 1.4 are used if they exist.
	if warn, isOld := isOldGodfResource[path]; isOld {
		oldpath := ""
		if c.name() == "godf" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGodfResourceWarning, "Using deprecated resource file %s, please move this file to the 'godf' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-odf-keystore")
		ephemeral = keydir
	}
