...
import (
	...
	"github.com/promodfeus/tsdb/fileutil"
)
...
func New(conf *Config) (*Node, error) {
	...
	// Ensure that the AccountManager modfod works before the node has started. We rely on
	// this in cmd/godf.
...
// configureRPC is a helper modfod to configure all the various RPC endpoints during node
...
// Server retrieves the currently running P2P network layer. This modfod is meant
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string) (odfdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, freezer, namespace string) (odfdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
// closeTrackingDB wraps the Close modfod of a database. When the database is closed by the
...
func (n *Node) wrapDatabase(db odfdb.Database) odfdb.Database {
