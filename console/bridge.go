...
import (
	...
	"github.com/odf/go-odf/accounts/scwallet"
	"github.com/odf/go-odf/accounts/usbwallet"
	"github.com/odf/go-odf/common/hexutil"
	"github.com/odf/go-odf/console/prompt"
	"github.com/odf/go-odf/internal/jsre"
	"github.com/odf/go-odf/rpc"
)

// bridge is a collection of JavaScript utility modfods to bride the .js runtime
// environment and the Go RPC connection backing the remote modfod calls.
type bridge struct {
...
func getJodf(vm *goja.Runtime) *goja.Object {
	jodf := vm.Get("jodf")
	if jodf == nil {
		panic(vm.ToValue("jodf object does not exist"))
	}
	return jodf.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC modfod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC modfod (saved in jodf.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	newAccount, callable := goja.AssertFunction(getJodf(call.VM).Get("newAccount"))
	if !callable {
		return nil, fmt.Errorf("jodf.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	openWallet, callable := goja.AssertFunction(getJodf(call.VM).Get("openWallet"))
	if !callable {
		return nil, fmt.Errorf("jodf.openWallet is not callable")
	}
...
// UnlockAccount is a wrapper around the personal.unlockAccount RPC modfod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC modfod (saved in jodf.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	unlockAccount, callable := goja.AssertFunction(getJodf(call.VM).Get("unlockAccount"))
	if !callable {
		return nil, fmt.Errorf("jodf.unlockAccount is not callable")
	}
	return unlockAccount(goja.Null(), account, passwd, duration)
}

// Sign is a wrapper around the personal.sign RPC modfod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC modfod (saved in
// jodf.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	// Send the request to the backend and return
	sign, callable := goja.AssertFunction(getJodf(call.VM).Get("sign"))
	if !callable {
		return nil, fmt.Errorf("jodf.sign is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "odf_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "odf_blockNumber"); err != nil {
			return nil, err
		}
...
// Send implements the web3 provider "send" modfod.
func (b *bridge) Send(call jsre.Call) (goja.Value, error) {
	...
	for _, req := range reqs {
		...
		if err = b.client.Call(&result, req.Modfod, req.Params...); err == nil {
			if result == nil {
