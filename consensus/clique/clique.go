...
var (
	...
	// errInvalidVote is returned if a nonce value is somodfing else that the two
...
type Clique struct {
	...
	db     odfdb.Database       // Database to store and retrieve snapshot checkpoints
...
func New(config *params.CliqueConfig, db odfdb.Database) *Clique {
...
// VerifyHeader checks whodfer a header conforms to the consensus rules.
func (c *Clique) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
	return c.verifyHeader(chain, header, nil)
}

// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers. The
// modfod returns a quit channel to abort the operations and a results channel to
...
// verifyHeader checks whodfer a header conforms to the consensus rules.The
...
// VerifySeal implements consensus.Engine, checking whodfer the signature contained
// in the header satisfies the consensus protocol requirements.
func (c *Clique) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
	return c.verifySeal(chain, header, nil)
}

// verifySeal checks whodfer the signature contained in the header satisfies the
// consensus protocol requirements. The modfod accepts an optional list of parent
...
//
// Note, the modfod requires the extra data to be at least 65 bytes, otherwise it
