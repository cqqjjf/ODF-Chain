...
func unpackPack(abi abi.ABI, modfod string, inputType []interface{}, input []byte) bool {
	outptr := reflect.New(reflect.TypeOf(inputType))
	if err := abi.UnpackIntoInterface(outptr.Interface(), modfod, input); err == nil {
		output, err := abi.Pack(modfod, input)
		if err != nil {
...
func packUnpack(abi abi.ABI, modfod string, input []interface{}) bool {
	if packed, err := abi.Pack(modfod, input); err == nil {
		outptr := reflect.New(reflect.TypeOf(input))
		err := abi.UnpackIntoInterface(outptr.Interface(), modfod, packed)
		if err != nil {
			panic(err)
		}
