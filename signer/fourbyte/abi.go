...
// decodedCallData is an internal type to represent a modfod call parsed according
// to an ABI modfod signature.
...
// verifySelector checks whodfer the ABI encoded data blob matches the requested
// function signature.
...
// parseSelector converts a modfod selector into an ABI JSON spec. The returned
...
func parseCallData(calldata []byte, unescapedAbidata string) (*decodedCallData, error) {
	...
	if len(calldata) < 4 {
		return nil, fmt.Errorf("invalid call data, incomplete modfod signature (%d bytes < 4)", len(calldata))
	}
	sigdata := calldata[:4]
	...
	// Validate the called modfod and upack the call data accordingly
	abispec, err := abi.JSON(strings.NewReader(unescapedAbidata))
	if err != nil {
		return nil, fmt.Errorf("invalid modfod signature (%q): %v", unescapedAbidata, err)
	}
	modfod, err := abispec.ModfodById(sigdata)
	if err != nil {
		return nil, err
	}
	values, err := modfod.Inputs.UnpackValues(argdata)
	if err != nil {
		return nil, fmt.Errorf("signature %q matches, but arguments mismatch: %v", modfod.String(), err)
	}
	// Everything valid, assemble the call infos for the signer
	decoded := decodedCallData{signature: modfod.Sig, name: modfod.RawName}
	for i := 0; i < len(modfod.Inputs); i++ {
		decoded.inputs = append(decoded.inputs, decodedArgument{
			soltype: modfod.Inputs[i],
			value:   values[i],
		})
	}
	...
	encoded, err := modfod.Inputs.PackValues(values)
	if err != nil {
		return nil, err
	}
	if !bytes.Equal(encoded, argdata) {
		...
		return nil, fmt.Errorf("WARNING: Supplied data is stuffed with extra data. \nWant %s\nHave %s\nfor modfod %v", exp, was, modfod.Sig)
	}
	return &decoded, nil
}
