...
type downloadTester struct {
	...
	stateDb odfdb.Database // Database used by the tester for syncing from peers
	peerDb  odfdb.Database // Database of the peers containing all data
...
// This modfod assumes that the caller holds at least the read-lock (dl.lock)
func (dl *downloadTester) getHeaderByHash(hash common.Hash) *types.Header {
...
// RequestBodies constructs a getBlockBodies modfod associated with a particular
...
// RequestReceipts constructs a getReceipts modfod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
// RequestNodeData constructs a getNodeData modfod associated with a particular
// peer in the download tester. The returned function can be used to retrieve
...
func assertOwnChain(t *testing.T, tester *downloadTester, length int) {
	// Mark this modfod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", odf.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", odf.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", odf.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", odf.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want odf.SyncProgress) {
	// Mark this modfod as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", odf.SyncProgress{
