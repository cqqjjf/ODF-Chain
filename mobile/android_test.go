...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Godf.LightScryptN, Godf.LightScryptP);
		...
		try {
			...
			// data from this modfod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.odf", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-odf issue #14599.
...
//
// This modfod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogodfer
	...
	workspace, err := ioutil.TempDir("", "godf-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/odf/godftest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Godf and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.odf", "github.com/odf/go-odf/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "godf.aar"), "godf.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "odf", "godftest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
