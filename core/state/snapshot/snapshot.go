...
// additional modfods compared to the public API.
type snapshot interface {
	Snapshot
	...
	// reached.
	//
	// Note, the modfod is an internal helper to avoid type switching between the
	...
	//
	// Note, the maps are retained by the modfod to avoid copying everything.
	...
	// Stale return whodfer this layer has become stale (was flattened across) or
	// if it's still live.
	Stale() bool
...
type Tree struct {
	diskdb odfdb.KeyValueStore      // Persistent database to store the snapshot
...
// background thread.
func New(diskdb odfdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool) *Tree {
...
// waitBuild blocks until the snapshot finishes rebuilding. This modfod is meant
...
//
// The modfod returns the new disk layer if diffs were persistend into it.
...
// it. The modfod will panic if called onto a non-bottom-most diff layer.
...
// flattening everything down (bad for reorgs).
//
// The modfod returns the root hash of the base layer that needs to be persisted
...
func (t *Tree) Rebuild(root common.Hash) {
	...
	// Track whodfer there's a wipe currently running and keep it alive if so
