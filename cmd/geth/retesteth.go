...
var (
	...
	retestodfCommand = cli.Command{
		Action:      utils.MigrateFlags(retestodf),
		Name:        "retestodf",
		Usage:       "Launches godf in retestodf mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches godf in retestodf mode (no database, no network, only retestodf RPC interface)`,
	}
)

type RetestodfTestAPI interface {
...
type RetestodfEthAPI interface {
...
type RetestodfDebugAPI interface {
...
type RetestodfAPI struct {
	odfDb         odfdb.Database
	db            state.Database
...
func (api *RetestodfAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	// Clean up
	...
	chainConfig, genesisHash, err := core.SetupGenesisBlock(odfDb, genesis)
	if err != nil {
		return false, err
	}
	...
	case "NoProof", "NoReward":
		inner = odfash.NewFaker()
	case "Ethash":
		inner = odfash.New(odfash.Config{
			CacheDir:         "odfash",
			CachesInMem:      2,
			CachesOnDisk:     3,
	...
	blockchain, err := core.NewBlockChain(odfDb, nil, chainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
		return false, err
	}
	...
	api.db = state.NewDatabase(api.odfDb)
...
func (api *RetestodfAPI) SendRawTransaction(ctx context.Context, rawTx hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestodfAPI) MineBlocks(ctx context.Context, number uint64) (bool, error) {
...
func (api *RetestodfAPI) currentNumber() uint64 {
...
func (api *RetestodfAPI) mineBlock() error {
	number := api.currentNumber()
	parentHash := rawdb.ReadCanonicalHash(api.odfDb, number)
	parent := rawdb.ReadBlock(api.odfDb, parentHash, number)
	...
	// If we are care about TheDAO hard-fork check whodfer to override the extra-data or not
	if daoBlock := api.chainConfig.DAOForkBlock; daoBlock != nil {
		// Check whodfer the block is among the fork extra-override range
		limit := new(big.Int).Add(daoBlock, params.DAOForkExtraRange)
		if header.Number.Cmp(daoBlock) >= 0 && header.Number.Cmp(limit) < 0 {
			// Depending whodfer we support or oppose the fork, override differently
...
func (api *RetestodfAPI) importBlock(block *types.Block) error {
...
func (api *RetestodfAPI) ModifyTimestamp(ctx context.Context, interval uint64) (bool, error) {
	api.blockInterval = interval
	return true, nil
}

func (api *RetestodfAPI) ImportRawBlock(ctx context.Context, rawBlock hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestodfAPI) RewindToBlock(ctx context.Context, newHead uint64) (bool, error) {
...
func (api *RetestodfAPI) GetLogHash(ctx context.Context, txHash common.Hash) (common.Hash, error) {
	receipt, _, _, _ := rawdb.ReadReceipt(api.odfDb, txHash, api.chainConfig)
	if receipt == nil {
...
func (api *RetestodfAPI) BlockNumber(ctx context.Context) (uint64, error) {
	return api.currentNumber(), nil
}

func (api *RetestodfAPI) GetBlockByNumber(ctx context.Context, blockNr math.HexOrDecimal64, fullTx bool) (map[string]interface{}, error) {
...
func (api *RetestodfAPI) GetBlockByHash(ctx context.Context, blockHash common.Hash, fullTx bool) (map[string]interface{}, error) {
...
func (api *RetestodfAPI) AccountRange(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	addressHash *math.HexOrDecimal256, maxResults uint64,
) (AccountRangeResult, error) {
	var (
...
func (api *RetestodfAPI) GetBalance(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (*math.HexOrDecimal256, error) {
...
func (api *RetestodfAPI) GetCode(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (hexutil.Bytes, error) {
	header := api.blockchain.GetHeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return nil, err
...
func (api *RetestodfAPI) GetTransactionCount(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (uint64, error) {
	header := api.blockchain.GetHeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return 0, err
...
func (api *RetestodfAPI) StorageRangeAt(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	address common.Address,
	begin *math.HexOrDecimal256, maxResults uint64,
) (StorageRangeResult, error) {
...
func (api *RetestodfAPI) ClientVersion(ctx context.Context) (string, error) {
	return "Godf-" + params.VersionWithCommit(gitCommit, gitDate), nil
}

func retestodf(ctx *cli.Context) error {
	log.Info("Welcome to retestodf!")
	// register signer API with server
	var (
		extapiURL string
	)
	apiImpl := &RetestodfAPI{}
	var testApi RetestodfTestAPI = apiImpl
	var odfApi RetestodfEthAPI = apiImpl
	var debugApi RetestodfDebugAPI = apiImpl
	...
	srv := rpc.NewServer()
	err := node.RegisterApisFromWhitelist(rpcAPI, []string{"test", "odf", "debug", "web3"}, srv, false)
	if err != nil {
	...
	// start http server
	var RetestodfHTTPTimeouts = rpc.HTTPTimeouts{
		ReadTimeout:  120 * time.Second,
		WriteTimeout: 120 * time.Second,
		IdleTimeout:  120 * time.Second,
	}
	httpEndpoint := fmt.Sprintf("%s:%d", ctx.GlobalString(utils.HTTPListenAddrFlag.Name), ctx.Int(rpcPortFlag.Name))
	httpServer, _, err := node.StartHTTPEndpoint(httpEndpoint, RetestodfHTTPTimeouts, handler)
	if err != nil {
