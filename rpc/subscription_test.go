...
func TestSubscriptions(t *testing.T) {
	...
	for i, namespace := range namespaces {
		request := map[string]interface{}{
			"id":      i,
			"modfod":  fmt.Sprintf("%s_subscribe", namespace),
			"version": "2.0",
...
func TestServerUnsubscribe(t *testing.T) {
	...
	p2.Write([]byte(`{"jsonrpc":"2.0","id":1,"modfod":"nftest2_subscribe","params":["someSubscription",0,10]}`))
	...
	p2.Write([]byte(`{"jsonrpc":"2.0","modfod":"nftest2_unsubscribe","params":["` + sub.subid + `"]}`))
	for {
		select {
