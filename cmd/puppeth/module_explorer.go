...
FROM puppodf/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'godf --cache 512 init /genesis.json' > explorer.sh && \
  echo $'godf --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --odfstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,odf,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec godf --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --odfstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,odf,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"EthPort":     config.node.port,
		"EthName":     config.node.odfstats[:strings.Index(config.node.odfstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":       info.node.odfstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.odf"],
			port:     infos.portmap[infos.envvars["ETH_PORT"]+"/tcp"],
			odfstats: infos.envvars["ETH_NAME"],
		},
