...
// filter is identical to Filter, but **does not** regenerate the heap. This modfod
...
// Remove deletes a transaction from the maintained map, returning whodfer the
...
// prevent getting into and invalid state. This is not somodfing that should ever
...
type txList struct {
	strict bool         // Whodfer nonces are strictly continuous or not
...
// Overlaps returns whodfer the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whodfer the
...
// returned.
//
// This modfod uses the cached costcap and gascap to quickly decide if there's even
...
// Remove deletes a transaction from the maintained list, returning whodfer the
...
// Empty returns whodfer the list of transactions is empty or not.
...
// Underpriced checks whodfer a transaction is cheaper than (or as cheap as) the
