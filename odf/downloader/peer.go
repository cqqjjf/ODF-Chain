...
// LightPeer encapsulates the modfods required to synchronise with a remote light peer.
...
// Peer encapsulates the modfods required to synchronise with a remote full peer.
type Peer interface {
	LightPeer
...
// lightPeerWrapper wraps a LightPeer struct, stubbing out the Peer-only modfods.
...
// Lacks retrieves whodfer the hash of a blockchain item is on the peers lacking
// list (i.e. whodfer we know that the peer does not have it).
...
//
// The modfod also sets the starting throughput values of the new peer to the
