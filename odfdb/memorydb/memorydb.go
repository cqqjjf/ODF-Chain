...
// New returns a wrapped map with all the required database interface modfods
// implemented.
func New() *Database {
	return &Database{
...
// all the required database interface modfods implemented.
...
func (db *Database) NewBatch() odfdb.Batch {
	return &batch{
		db: db,
	}
}
...
func (db *Database) NewIterator(prefix []byte, start []byte) odfdb.Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
//
// Note, this modfod is only used for testing (i.e. not public in general) and
...
func (b *batch) Replay(w odfdb.KeyValueWriter) error {
	for _, keyvalue := range b.writes {
		if keyvalue.delete {
...
// Next moves the iterator to the next key/value pair. It returns whodfer the
